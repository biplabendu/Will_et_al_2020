...
# Enrichment analysis (hypergeometric test): A tutorial
...
```r
all_genes <- read.csv("./FullBlast_EC05_RNAseq_orignal_copy_26Aug19.csv", header = T, stringsAsFactors = FALSE) # user-specified annotation file
```
...
```r
# load test geneset to run enrichment on
ophio_rnaseq <- read.csv("./enrichtest_fungal.csv", stringsAsFactors = F, header = T) # user-specified test geneset
```
...
# Customizing R script for use with any organism.
- Replace the "user-specified annotation file" and "user-specified test geneset" with your own files.
- The column names in the user-specified annotation file should match the ones used in the function.
  - i.e., the column names should be "GO","PFAM","signalp", "TMHMM" (case-sensitive)
- Multiple (GO/PFAM) annotations should be formatted as shown above.
  - e.g. the GO annotations for a gene should read "GO:0005488|binding; GO:0008270|zinc ion binding; GO:0046872|metal ion binding; GO:0003674|molecular_function;"
